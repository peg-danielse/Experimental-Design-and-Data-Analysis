...
To verify the claim we do a t-test with H0: μ = 2800 and Ha: μ \> 2800.
The result of the t-test has a p-value of 0.014, which is smaller than
alpha. This means we can reject H0, and Ha is accepted. The test also
tells us that there is a probability of 95% that the mean is greater
than 2819.20. For an appropriate sign-test we check the number of
observations that are greater than 2800 and the total number of
observations in the data set. With H0: μ = 2800 and Ha: μ \> 2800. The
...
-   h0: p(after) = unif(3,θ) where θ in [3, 12]

-   h1: p(after) ≠ unif(3,θ) where θ in [3, 12]

We performed the bootstrap test for the values for θ in 3 ... 12. For
the values inside the bootstrapped confidence interval we cannot reject
the null hypothesis.
...
after 6 weeks. There seems to be a difference. we check normality for
the difference between the pre-weight and the after 6 weeks weight this
seems to be normal. Lastly, we want to investigate if there is a
significant difference between the mean weight before and after. We
perform a T-test with significance level of 95% and hypotheses:
...
-   Alternative hypothesis: The mean of at least one diet-type is not
...
subquestion a. Diet 3 has the biggest effect on the weight lost.
...
```{r}
dietframe <- data.frame(weight=(diet$preweight-diet$weight6weeks), diet=factor(diet$diet))
...
```{r}
dgframe <- data.frame(weight=diet$preweight - diet$weight6weeks, diet=factor(diet$diet), gender=factor(diet$gender))
...
```{r}
par(mfrow=c(1,2))
interaction.plot(npk$N, npk$block, npk$yield, xlab = "nitrogen", ylab = "yield", trace.label = "block")
interaction.plot(npk$block, npk$N, npk$yield, xlab = "block", ylab = "yield", trace.label = "nitrogen")
```
...
reject H0 and accept Ha. It is not sensible to include the block factor
in the test because the yield of the block is dependent of what
additives is received by each plot in the block. For a Friedman test the
variable need a similar spread across the different variable groups.
However there is not necessarily a similar spread between blocks and
nitrogen so we cannot apply a Friedman test in this case.
