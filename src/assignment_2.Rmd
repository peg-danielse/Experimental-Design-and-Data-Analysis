...
To investigate the differences in tree types we want to use a t-test. You can 
only perform a t-test if the data contains two groups. Which is
the case for the tree volumes since there are two types of trees. Therefore,
it is similar to the T-test in the results but not in the statistic used for
arriving at the result since anova uses an f distributed statistic.
...
We cannot improve this confident interval since we removed the influence points 
already.
...
xtabs(~PClass + Sex, data=titanic)

tot = xtabs(~PClass + Age + Survived, data=titanic)
hist(tot)

xtabs(Survived~Age + PClass, data=titanic)
